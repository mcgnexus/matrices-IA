...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Variables de entorno:', {
      ...
      googleKeyLength: GOOGLE_API_KEY ? GOOGLE_API_KEY.length : 0
    });
    
    if (!GOOGLE_API_KEY) {
      console.error('Variable de entorno faltante: GOOGLE_API_KEY');
      return res.status(500).json({ 
        error: 'Configuración de API no encontrada',
        details: 'Falta la variable GOOGLE_API_KEY',
    ...
    // Realizar petición a la API de Gemini con timeout
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          userMessage = 'API Key inválida o sin permisos';
